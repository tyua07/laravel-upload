...
class SliceUploading {
    ...
    public function __construct($timeoutMs, $maxRetryCount) {
        ...
        $this->errorCode = Response::COSAPI_SUCCESS;
        $this->errorMessage = '';
    ...
    public function performUploading() {
        for ($i = 0; $i < $this->concurrentTaskNumber; ++$i) {
            ...
            if ($sliceContent === false) {
                $this->setError(Response::COSAPI_PARAMS_ERROR, 'read file ' . $this->srcFpath . ' error');
                return false;
            }
        ...
        if ($this->errorCode !== Response::COSAPI_SUCCESS) {
            return false;
        }
    ...
    private function sendRequest($request) {
        ...
        if ($response === false) {
            $this->setError(Response::COSAPI_NETWORK_ERROR, 'network error');
            return false;
        }
    ...
    public function uploadCallback($request, $response) {
        ...
        $requestErrorCode = Response::COSAPI_SUCCESS;
        $requestErrorMessage = 'success';
        $retryCount = $request->userData['retryCount'];

        $responseJson = json_decode($response->body, true);
        if ($responseJson === NULL) {
            $requestErrorCode = Response::COSAPI_NETWORK_ERROR;
        ...
        if (isset($responseJson['data']['datamd5']) &&
            ...
            $requestErrorCode = Response::COSAPI_INTEGRITY_ERROR;
            $requestErrorMessage = 'cosapi integrity error';
        }

        if ($requestErrorCode !== Response::COSAPI_SUCCESS) {
