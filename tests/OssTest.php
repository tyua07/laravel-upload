...
class OssTest extends TestCase
{
     ...
     * 上传引擎
     *
     * @var \Yangyifan\Upload\Oss\OssAdapter
     */
    ...
    {
        $ossConfig      = array_merge(config(require base_path('/config.php'), 'oss'), require_once base_path('/ossConfig.php'));
        static::$drive  = new Yangyifan\Upload\Oss\OssAdapter($ossConfig);
        static::$config = new \League\Flysystem\Config($ossConfig);
...
//        $this->assertEquals($timestamp2, '1480663159');
    ...
    {
        $this->assertTrue(static::$drive->createDir('/test/11212', static::$config), true);
    }
